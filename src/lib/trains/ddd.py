...
from models.losses import FocalLoss, L1Loss, BinRotLoss, RegWeightedL1Loss
...
class DddLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      vertex_coordinate_loss +=  self.crit_kp(output['vertex_coordinate'],                    batch['vertex_coordinate_mask'],batch['ind'], batch['vertex_coordinate']) / opt.num_stacks
    ...
    loss = opt.hm_weight * hm_loss + opt.dep_weight * dep_loss + \
           ...
           vertex_hm_loss + opt.vertex_coordinate_weight * vertex_coordinate_loss +  opt.vertex_off_weight * vertex_off_loss
...
class DddTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'dep_loss', 'dim_loss', 'rot_loss', 
                   'wh_loss', 'off_loss','vertex_hm_loss','vertex_coordinate_loss','vertex_off_loss']
    loss = DddLoss(opt)
