...
class DddDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    vertex_reg_offset = np.zeros(
        (self.max_objs, 18), dtype=np.float32)
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        if 'vertex' in ann.keys():
          ...
          vertexes = np.zeros((9,2),dtype=np.float32)
         #  do affine transform for gt vertexes
        for i,vertex in enumerate(vertexes.tolist()):
          vertexes[i] = affine_transform(vertex, trans_output)
          vertexes[i,0] = np.clip(vertexes[i,0], 0, self.opt.output_w - 1)
          vertexes[i,1] = np.clip(vertexes[i,1], 0, self.opt.output_h - 1)
          # gaussian in vertexes heatmap
        for i,vertex in enumerate(vertexes):
#           print(vertexes)
#           print(vertexes[i])
          draw_gaussian(vertex_hm[i], vertexes[i], radius)
        # compute offset between 2D bbox center and projected vertex coordinates
        vertex_coordinate[k] = (vertexes - ct).flatten()
        ...
        if 1:
          ...
          vertex_ind[k] = (vertexes_int[:,1] * self.opt.output_w + vertexes_int[:,0]).flatten()
          reg[k] = ct - ct_int
          vertex_reg_offset[k] = (vertexes - vertexes_int).flatten()
