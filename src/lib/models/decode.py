...
# 还需要把vertex heatmap的score给返回；
def ddd_decode(heat, rot, depth, dim,vertex_coordinate,vertex_hm, vertex_reg_offset=None, wh=None, reg=None, K=40):
    ...
    vertex_coordinate = vertex_coordinate.permute(0, 2, 1, 3).contiguous().view(
          batch, K, 9 * 2)
    # add vertex heatmap scores to detections
    hm_score = hm_score.permute(3, 0, 2, 1).squeeze(0) # b x K x J
    # detections = torch.cat([xs, ys, scores, rot, depth, dim, wh, vertex_coordinate, clses], dim=2)
    detections = torch.cat([xs, ys, scores, rot, depth, dim, wh, vertex_coordinate, hm_score, clses], dim=2)
    return detections
