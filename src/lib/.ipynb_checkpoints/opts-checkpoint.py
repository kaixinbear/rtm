...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--arch', default='dla_34', 
                             help='model architecture. Currently tested'
                                  'res_18 | res_101 | resdcn_18 | resdcn_101 | resFP_18 |'
    ...
    self.parser.add_argument('--vertex_off_weight', type=float, default=0.5,
                             help='loss weight for vertex local offsets.')
    self.parser.add_argument('--vertex_coordinate_weight', type=float, default=0.1,
                             help='loss weight for vertex local offsets.')
    ...
    self.parser.add_argument('--rot_weight', type=float, default=0.5,
                             help='loss weight for orientation.')
    self.parser.add_argument('--peak_thresh', type=float, default=0.4)
    ...
    self.parser.add_argument('--not_vertex_reg_offset', action='store_true',
                             help='not regression vertex offset.')
  ...
  def parse(self, args=''):
    ...
    opt.vertex_reg_offset = not opt.not_vertex_reg_offset
...
#       opt.heads = {'hm': opt.num_classes, 'dep': 1, 'rot': 8, 'dim': 3}
      # add vertex part
      opt.heads = {'hm': opt.num_classes,'vertex_hm':9,
                   'vertex_coordinate':18, 'dep': 1, 'rot': 8, 'dim': 3}
      if opt.reg_bbox:
        opt.heads.update(
          {'wh': 2})
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.vertex_reg_offset:
        opt.heads.update({'vertex_reg_offset': 2})
    elif opt.task == 'ctdet':
