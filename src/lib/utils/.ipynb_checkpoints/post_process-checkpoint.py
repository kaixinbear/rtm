...
detections = torch.cat(
            [xs, ys, scores, rot, depth, dim,   wh, vertex_coordinate, clses], dim=2)
            0    1    2      [3:11)  11  12:15  15:17   17:35            35
'''
def ddd_post_process_2d(dets, c, s, opt):
  ...
  for i in range(dets.shape[0]):
    ...
    dets[i, :, 17:35] =  transform_preds(
            dets[i, :, 17:35].reshape(-1, 2), c[i], s[i], (opt.output_w, opt.output_h)).reshape(-1, 18)
    classes = dets[i, :, -1]
    for j in range(opt.num_classes):
      ...
      if include_wh:
        top_preds[j + 1] = np.concatenate([
          top_preds[j + 1],
          transform_preds(dets[i, inds, 15:17], c[i], s[i], (opt.output_w, opt.output_h)).astype(np.float32),
          dets[i, inds, 17:35].astype(np.float32),
          dets[i, inds, 35:44].astype(np.float32)],axis=1) # add scores
  ...
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        hm_score = dets[i][cls_ind][j][28:37]
        # 此处有待改进，center是2D bbox的center，但是用来确定了3D bbox的中心点；
#         if hm_score[-1] > 0.4:
#             print("use real 3D bbox center!")
#             print("center",center,"vertex",dets[i][cls_ind][j][[18,27]])
#             locations, rotation_y = ddd2locrot(
#               dets[i][cls_ind][j][[18,27]], alpha, dimensions, depth, calibs[0])
#             score = hm_score[-1]
#         else:
#             locations, rotation_y = ddd2locrot(
#               center, alpha, dimensions, depth, calibs[0])  
        ...
        # -------------------调用优化能量函数------------------------------------------------
        if score>0.4 and np.all(hm_score>0.3):
            h, w, l = dimensions
            cx3d, cy3d, cz = locations
            input1 = np.array([w,h,l,rotation_y,cx3d, cy3d, cz])
            input2 = np.array(dets[i][cls_ind][j][10:28])
            input3 = np.array([calibs[0][0,0], calibs[0][0,2], calibs[0][1,2]])
            # 应该把协方差矩阵(置信度)传入；
            optimized_result = optimize(input1,input2,input3,score,hm_score)
            # dimensions的误差很大 ,所以不对dimensions做优化           
            # dimensions = np.array([optimized_result[1],optimized_result[0],optimized_result[2]])
            optimized_rotation_y = np.array(optimized_result[3])
            optimized_locations = np.array([optimized_result[4],optimized_result[5],optimized_result[6]])
            rotation_y = optimized_rotation_y
